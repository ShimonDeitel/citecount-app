import XCTest
@testable import Citecount

@MainActor
final class CitecountTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        store.add(Source(title: "Test", cited: false))
        XCTAssertEqual(store.items.count, before + 1)
    }

    func testDeleteRemovesItem() {
        let store = makeStore()
        let item = Source(title: "Test", cited: false)
        store.add(item)
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        ...
        for i in 0..<Store.freeLimit {
            store.add(Source(title: "Item \(i)", cited: false))
        }
